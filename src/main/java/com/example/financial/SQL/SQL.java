...
public class SQL {
    ...
    private static List<Type> data = new ArrayList<>();
    private static List<Type> History = new ArrayList<>();
    ...
    public List<Type> getList(LocalDate lcd) throws SQLException {
        final String findDate = "select type, money, imageSource, buttonColor from data where time = ?";
        pst = connect.prepareStatement(findDate);
        pst.setDate(1, Date.valueOf(lcd));
    ...
    /** xóa type. */
    public void deleteType(final String type) throws SQLException {
        final String delType = "delete from data where type = ?";
    ...
    /** thêm giao dịch. */
    public void addType(final String type, final int money, final String imageSource, final String buttonColor, final LocalDate lcd) throws SQLException {
        ...
        pst.setDate(5, Date.valueOf(lcd));
        try {
            pst.executeUpdate();
    ...
    public List<Type> getHistory(int month, int year) throws SQLException {
        final String count = "select money from history where extract(month from time) = ? and extract(year from time) = ?";
        pst = connect.prepareStatement(count);
        pst.setInt(1, month);
        pst.setInt(2, year);
        ResultSet rs = pst.executeQuery();
